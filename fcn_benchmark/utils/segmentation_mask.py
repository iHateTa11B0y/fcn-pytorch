...
class PolygonInstance(object):
    ...
    def transpose(self, method):
        if method not in (FLIP_LEFT_RIGHT, FLIP_TOP_BOTTOM, TRANSPOSE):
        ...
        if method == TRANSPOSE:
            for poly in self.polygons:
                p = poly.clone()
                p = p.reshape(-1,2)[:,(1,0)].reshape(1,-1).squeeze()
                flipped_polygons.append(p)
            return Polygons(flipped_polygons, size=(height, width), mode=self.mode)
